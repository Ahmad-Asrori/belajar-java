...
class Kursi implements Comparable<Kursi>{
    ...
    public Kursi(String kodeKursi, float harga) {
...
public class Teater {
    ...
    static final Comparator<Kursi> HARGA_KURSI = new Comparator<Kursi>() {
        @Override
        public int compare(Kursi o1, Kursi o2) {
    ...
    /*
        ...
        step 5 (baris ke ) : beri harga
        step 6 (baris ke 59) : buat objek kursi lalu masukkan baris kursi dan nomor kursi
        step 7 (baris ke ) : masukkan objek kursi yang sudah berisi kode kursi kedalam arraylist
    */
    public Teater(String namaTeater, int jumlahBaris, int kursiPerBaris) {
        ...
        for (char baris = 'A'; baris <= barisTerakhir; baris++) {
            for(int nomorKursi = 1; nomorKursi <= kursiPerBaris; nomorKursi++) {
                ...
                if (baris < 'D' && (nomorKursi>=4 && nomorKursi <= 9)){
                    harga = 50000;
                } else if (baris > 'F' || nomorKursi < 4 || nomorKursi > 9){
                    harga = 25000;
                }

                Kursi kursi = new Kursi(baris + String.format("%02d", nomorKursi), harga);
    ...
    public boolean pesamKursi(String kodeKursi) {
        Kursi kursiYangDiminta = new Kursi(kodeKursi, 0);
...
/*
    class ini untuk sorting 2 objek
 */
class harga implements Comparator<Kursi> {
    @Override
    public int compare(Kursi o1, Kursi o2) {
        if (o1.getHarga() < o2.getHarga()){
            return -1;
